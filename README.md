...
⚠️ **이 저장소 루트(`GoldenLock-main/`)엔 `contracts`/`relay-server`/`zk-circuits` 세 개의
독립된 npm 프로젝트가 들어있다.** 루트에서 그냥 `npm install`을 치면 세 프로젝트를 순서대로
설치해준다 (아래 `npm install` 한 줄로 대체 가능):

```bash
npm install   # == npm install --prefix contracts && npm install --prefix relay-server && npm install --prefix zk-circuits
```

또는 원래대로 폴더별로 따로 들어가서 설치해도 된다:
...
# 4) ai-engine (구급대 콘솔의 "AI로 자동 산출" 버튼이 호출함 — frontend보다 먼저 켜둘 것)
cd ../ai-engine && python -m pip install -r requirements.txt
uvicorn main:app --reload --port 8000

# 5) frontend
cd ../frontend && python3 -m http.server 5173
# 브라우저 접속 → 우측 상단 "설정"에서 Relay URL / RPC URL / 컨트랙트 주소 / AI Engine URL 입력
...
1. **구급대 콘솔**에서 활력징후/증상을 입력해 "AI로 자동 산출"을 누르면 브라우저가 직접
   `ai-engine`의 `/predict-ktas`를 호출해 KTAS 1~5 등급과 `requiredBeds`/`requiredSpecialists`를
   받아온다 (모델 미탑재 시 규칙 기반 폴백이 대신 응답하며, 이 사실이 `source`/`warning`
   필드로 그대로 노출된다).
2. 그 값으로 `relay-server`에 매칭을 요청한다. relay-server는 "지금 받을 수
...
- **`frontend/circuits/`에 컴파일된 ZK 산출물(.wasm/.zkey/.vkey)이 없다.** 소스만 있고
  빌드 결과물은 저장소에 커밋돼 있지 않으므로, `zk-circuits`에서 직접 컴파일해 복사하기
  전까지는 병원 콘솔의 "ZK 증명 생성" 버튼이 실패한다 (`zk-circuits/README.md`,
  `frontend/circuits/README.md` 참고).
...
- ai-engine에는 실제 응급실 트리아지 데이터셋(`data/raw/`, 약 35MB, Kaggle 스타일
  train/test)이 들어있고 `scripts/convert_triagegeist.py`로 학습용 포맷 변환도 가능하지만,
  저장소에 **학습된 `.onnx` 모델 파일 자체는 포함돼 있지 않다.** 그래서 지금은 항상
  `services/rule_based_fallback.py`(단순 임계값 규칙)로만 응답하며, API 응답의
  `source`/`warning` 필드가 이 사실을 그대로 드러낸다. 실제 학습(`services/train.py`,
  PyTorch 필요)과 ONNX export(`services/export_onnx.py`)를 직접 돌려야 진짜 모델이
  동작한다. 실제 임상 데이터·의료진 검증 없이는 진단·트리아지 목적으로 사용해서는 안
  된다 (`ai-engine/ARCHITECTURE.md` 참고).
- **폴더 이름에 `backend`/`blockchain`이 없다**: 원본 zip의 `backend/relay-server` +
  `blockchain/contracts`를 각각 `relay-server/`, `contracts/`로 통합·평탄화했다 (위
  "통합 결정 사항" 참고). 별도의 `backend/` 폴더를 찾고 있다면 그게 지금의
  `relay-server/`다.
