...
* multimodal_fusion.py의 sentence-transformers 텍스트 인코더는 온디바이스엔 무거우므로, 실배포 시엔 KoELECTRA-small 등을 양자화한 별도 ONNX 인코더로 교체

* AI-Hub에서 실제 응급 임상 데이터를 받아오려고 했으나, 신청 후 14일 뒤인 17일부터 사용 가능하여 포기 -> Kaggle "Triagegeist" 가상의 생성형 데이터로 대체

## 1. 책임 범위

`ai-engine`은 환자 입력을 KTAS 1~5 등급으로 분류하는 추론 서비스입니다. 분류 결과를 GoldenLock 컨트랙트/relay-server가 사용할 필요 병상·전문의 수(및 참고용 자원 코드)로 변환해 반환합니다.
...
- KTAS 결과의 필요 자원(병상/전문의) 변환

서비스 밖의 책임:

- 병원 자원 등록 및 실시간 매칭: `relay-server/`
- 자원 확약 및 온체인 상태: `contracts/`
- 실제 ZK 증명 생성/검증 회로: `zk-circuits/`, 브라우저측 생성은 `frontend/js/zk.js`
...
flowchart LR
    ...
    Map[Resource mapper\nKTAS -> requiredBeds/requiredSpecialists]
    Relay[relay-server\n병원 매칭 요청]
    ZK[ZK circuit / GoldenLock.sol\nrequestAndLock 공개 입력]
    ...
    API --> Map
    Map -->|response| Client
    Client -->|ktasGrade, requiredBeds, requiredSpecialists| Relay
    Relay --> ZK
```
...
6. `resource_mapper.map_ktas_to_resource`가 등급을 참고용 자원 코드·우선순위·`required_beds`·`required_specialists`로 변환합니다.
7. `required_beds`/`required_specialists`는 그대로 relay-server의 매칭 요청과 ZK 회로의 public input(`requiredBeds`, `requiredSpecialists`)으로 전달됩니다.
...
| KTAS 등급 | `resource_code`(참고용) | `required_beds` | `required_specialists` | 의미 |
|---:|---|---:|---:|---|
| 1 | `0x01` | 1 | 3 | 즉시 수술실 + 중환자실(소생) |
| 2 | `0x02` | 1 | 2 | 중환자실(긴급) |
| 3 | `0x03` | 1 | 1 | 응급병상 + 전문의(응급) |
| 4 | `0x04` | 1 | 0 | 일반병상(준응급) |
| 5 | `0x05` | 0 | 0 | 외래 대기(비응급) |

`required_beds`/`required_specialists`는 relay-server의 매칭 요청과 ZK 회로
(`hospital_resource.circom`)의 public input에 그대로 이어지는 서비스 간 계약입니다.
`ktas_grade`는 `GoldenLock.sol::requestAndLock`의 `ktasLevel`로 온체인에 기록됩니다.
`resource_code`/`resource_label`은 프론트엔드 표시용 참고 라벨일 뿐 온체인/회로 계약에는
관여하지 않습니다.
