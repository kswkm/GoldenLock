...
- 모델 입력 전처리
  ...
  - 증상 텍스트: `distiluse-base-multilingual-cased-v2` 임베딩, 768차원
...
`resource_code`와 `zk_public_input`은 백엔드 매칭 및 ZK 회로에서 사용할 값입니다.
...
relay-server는 이 `resource_code` 체계를 사용해 병원 자원 매칭 요청을 처리합니다. 서비스 간 계약이 변경되면 `services/resource_mapper.py`, relay-server 타입, ZK 회로의 공개 입력 정의를 함께 변경해야 합니다.
