...
function buildChallengeMessage(address, nonce, issuedAt) {
    return [
...
class WalletAuth {
    constructor() {
        this.challenges = new Map(); // key: socketId
        this.sessions = new Map(); // key: socketId
    }
    issueChallenge(socketId, address) {
    ...
    verify(socketId, address, signature) {
        ...
        let recovered;
        try {
            recovered = (0, ethers_1.verifyMessage)(message, signature);
        }
        catch {
            return false;
        }
    ...
    isAuthenticatedAs(socketId, address) {
