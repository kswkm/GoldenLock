...
exports.registerMatchSocket = registerMatchSocket;
const walletAuth_1 = require("../auth/walletAuth");
 ...
 */
const RELAYER_ACTION_WINDOW_MS = 60000;
const RELAYER_ACTION_MAX_CALLS = 10;
function registerMatchSocket(io, matchingService, relayer) {
    const walletAuth = new walletAuth_1.WalletAuth();
    const relayerActionLimiter = new rateLimiter_1.RateLimiter(RELAYER_ACTION_MAX_CALLS, RELAYER_ACTION_WINDOW_MS);
    /** 스키마 검증 헬퍼 — 실패 시 "validation:error"를 emit하고 null을 반환한다. */
    function parseOrReject(socket, event, schema, raw) {
    ...
    function requireAuth(socket, address, event) {
    ...
    function requireWithinRateLimit(socket, key, matchId) {
    ...
    io.on("connection", (socket) => {
        ...
        // ── 지갑 서명 인증 ────────────────────────────────────────
        socket.on("auth:request-challenge", (raw) => {
            const data = parseOrReject(socket, "auth:request-challenge", schemas_1.authRequestChallengeSchema, raw);
            if (!data)
                return;
            const challenge = walletAuth.issueChallenge(socket.id, data.address);
            socket.emit("auth:challenge", challenge);
        });
        socket.on("auth:verify", (raw) => {
            const data = parseOrReject(socket, "auth:verify", schemas_1.authVerifySchema, raw);
            if (!data)
                return;
        ...
        // ── 병원 관제 단말 ──────────────────────────────────────
        socket.on("hospital:join", async (raw) => {
            const data = parseOrReject(socket, "hospital:join", schemas_1.hospitalJoinSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.hospitalAddress, "hospital:join"))
                return;
        ...
        socket.on("hospital:update-availability", async (raw) => {
            const data = parseOrReject(socket, "hospital:update-availability", schemas_1.hospitalAvailabilityUpdateSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.hospitalAddress, "hospital:update-availability"))
                return;
            const registered = await safeIsRegisteredHospital(data.hospitalAddress);
            if (!registered) {
        ...
        // ── 구급대 단말 ──────────────────────────────────────────
        socket.on("ambulance:request-match", async (raw) => {
            const data = parseOrReject(socket, "ambulance:request-match", schemas_1.ambulanceMatchRequestSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.ambulanceAddress, "ambulance:request-match"))
                return;
            try {
        ...
        socket.on("ambulance:authorize-lock", (raw) => {
            const data = parseOrReject(socket, "ambulance:authorize-lock", schemas_1.authorizeLockSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.ambulanceAddress, "ambulance:authorize-lock"))
                return;
        ...
        socket.on("hospital:submit-request-proof", async (raw) => {
            const data = parseOrReject(socket, "hospital:submit-request-proof", schemas_1.submitRequestProofSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.hospitalAddress, "hospital:submit-request-proof"))
                return;
            const match = matchingService.getMatch(data.matchId);
            ...
            if (match.status !== types_1.MatchStatus.AUTHORIZED) {
            ...
            if (!requireWithinRateLimit(socket, `submit-proof:${data.hospitalAddress.toLowerCase()}`, data.matchId))
                return;
            // await 이전(동기 구간)에 상태를 선점해 동시 중복 제출을 막는다.
            matchingService.updateMatchStatus(match.matchId, types_1.MatchStatus.LOCK_REQUESTED);
            ...
            try {
                ...
                matchingService.updateMatchStatus(match.matchId, types_1.MatchStatus.LOCK_REQUESTED, { requestId, txHash });
                ...
                matchingService.updateMatchStatus(match.matchId, types_1.MatchStatus.AUTHORIZED);
        ...
        // ── 병원의 수용 확정 ─────────────────────────────────────
        socket.on("hospital:confirm-acceptance", async (raw) => {
            const data = parseOrReject(socket, "hospital:confirm-acceptance", schemas_1.confirmAcceptanceSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.hospitalAddress, "hospital:confirm-acceptance"))
                return;
            const match = matchingService.getMatch(data.matchId);
            ...
            if (match.status !== types_1.MatchStatus.LOCK_REQUESTED) {
                socket.emit("match:failed", { reason: "already-confirmed-or-not-ready", matchId: data.matchId });
                return;
            }
            if (!requireWithinRateLimit(socket, `confirm:${data.hospitalAddress.toLowerCase()}`, data.matchId))
                return;
            // await 이전에 상태를 선점해 더블클릭 등으로 인한 중복 확정 요청을 막는다.
            matchingService.updateMatchStatus(data.matchId, types_1.MatchStatus.LOCK_CONFIRMED);
            try {
                const txHash = await relayer.fulfillRequest(match.requestId, data.signature);
                matchingService.updateMatchStatus(data.matchId, types_1.MatchStatus.LOCK_CONFIRMED, { txHash });
                ...
                matchingService.updateMatchStatus(data.matchId, types_1.MatchStatus.LOCK_REQUESTED); // 재시도 가능하도록 되돌림
                socket.emit("match:failed", { reason: "confirm-tx-failed", matchId: data.matchId });
            }
        });
        // ── 이송 취소 / 완료 후 해제 ──────────────────────────────
        socket.on("match:release", async (raw) => {
            const data = parseOrReject(socket, "match:release", schemas_1.releaseSchema, raw);
            if (!data)
                return;
            if (!requireAuth(socket, data.ambulanceAddress, "match:release"))
                return;
            const match = matchingService.getMatch(data.matchId);
            ...
            if (!requireWithinRateLimit(socket, `release:${data.ambulanceAddress.toLowerCase()}`, data.matchId))
                return;
            // await 이전에 상태를 선점해 중복 취소 요청을 막는다.
            matchingService.updateMatchStatus(data.matchId, types_1.MatchStatus.RELEASED);
            try {
                const txHash = await relayer.cancelRequest(match.requestId, data.signature);
                matchingService.updateMatchStatus(data.matchId, types_1.MatchStatus.RELEASED, { txHash });
    ...
    async function safeIsRegisteredHospital(address) {
        try {
