...
export class MatchingService {
  ...
  findBestHospital(from: GeoPoint): HospitalRegistryEntry | null {
    let best: HospitalRegistryEntry | null = null;
    let bestDistance = Infinity;

    for (const hospital of this.hospitals.values()) {
      if (!hospital.hasCapacity) continue;
  ...
  setAmbulanceAuthorization(matchId: string, signature: string): MatchRecord | undefined {
    return this.updateMatchStatus(matchId, MatchStatus.AUTHORIZED, { ambulanceAuthSignature: signature });
  }
