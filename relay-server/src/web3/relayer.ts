...
export class Relayer {
  ...
  // relayer 지갑 하나를 여러 요청이 동시에 쓰면 nonce가 충돌하므로 트랜잭션 전송을 직렬화한다.
  private txQueue: Promise<unknown> = Promise.resolve();
  // JsonRpcProvider.getTransactionCount가 짧은 시간내 연속 호출 시 stale nonce를 반환하므로 직접 추적한다.
  ...
  private enqueue<T>(task: () => Promise<T>): Promise<T> {
    const result = this.txQueue.then(task, task);
  ...
  /** nonce를 직접 예약해 트랜잭션을 보내고, 실패하면 다음 호출 시 체인에서 nonce를 다시 동기화한다. */
  private async sendWithNonce(
  ...
  async submitAvailabilityProof(
    ...
    return this.enqueue(() =>
      this.sendWithNonce((nonce) =>
        this.contract.submitAvailabilityProof(resourceCode, isAvailable, proof.a, proof.b, proof.c, { nonce })
      )
    );
  }

  async submitAvailabilityProofFor(
    hospital: string,
    resourceCode: number,
    isAvailable: 0 | 1,
    proof: { a: [string, string]; b: [[string, string], [string, string]]; c: [string, string] },
    ...
    return this.enqueue(() =>
      this.sendWithNonce((transactionNonce) =>
        this.contract.submitAvailabilityProofFor(
          hospital,
          resourceCode,
          isAvailable,
          proof.a,
  ...
  async requestLock(params: {
    ...
    return this.enqueue(async () => {
      const receipt = await this.sendWithNonce((nonce) =>
        this.contract.requestLock(
  ...
  async confirmAcceptance(lockId: number): Promise<string> {
    return this.enqueue(async () => {
      const receipt = await this.sendWithNonce((nonce) => this.contract.confirmAcceptance(lockId, { nonce }));
      return receipt.hash;
    });
  }
  ...
  async releaseLock(lockId: number): Promise<string> {
    return this.enqueue(async () => {
      const receipt = await this.sendWithNonce((nonce) => this.contract.releaseLock(lockId, { nonce }));
      return receipt.hash;
    });
  }
  ...
  async expireLock(lockId: number): Promise<string> {
    return this.enqueue(async () => {
      const receipt = await this.sendWithNonce((nonce) => this.contract.expireLock(lockId, { nonce }));
      return receipt.hash;
    });
  }
  ...
  async availabilityNonce(hospitalAddress: string): Promise<number> {
    return Number(await this.contract.availabilityNonces(hospitalAddress));
  }
